...
async fn main() -> anyhow::Result<()> {
    ...
    for it in items {
        ...
        let icon_url = it.get("icon").and_then(|x| x.as_str())
            ...
            .unwrap_or_default();
        // Set 18 cdragon은 composition이 비어 있어 조합 여부를 알 수 없다.
        // 대신 id 규칙으로 판별한다 — 완성템만 표식 접두사가 없다.
        //   컴포넌트 DA_Component_*, 유물 DA_Artifact_*, 상징 *Emblem*, 찬란 *Radiant*
        let is_special = api.contains("_Component_")
            || api.contains("_Artifact_")
            || api.contains("Emblem")
            || api.contains("Radiant")
            || api.contains("Tacticians")      // Crown, Cape, Shield
            || api.contains("Potion");         // Health/Mana/Blast Potion
